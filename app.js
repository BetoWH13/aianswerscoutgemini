...
async function runScout(e) {
  ...
  try {
    ...
    $('runStatus').innerHTML = `<span class="error"><strong>Run failed:</strong> ${escapeHtml(err.message)}</span><br />Check Netlify environment variables and function logs. The most common issue is missing GEMINI_API_KEY. If Grounded Search failed, try Fast Scout first.`;
  } finally {
